...
class Response:
    ...
    @staticmethod
    def success(body: Union[dict, list] = {}) -> 'Response':
        return Response(status_code=200, body=body)

    @staticmethod
    def created_successfully(created_id: str = None) -> 'Response':
        return Response(status_code=201, body={'id': created_id} if created_id else {})

    @staticmethod
    def bad_request(message: str = None, validation_errors: List[str] = []) -> 'Response':
        messages = []
        if message:
            messages.append(message)
        if validation_errors:
            messages.append(f'Validation error: {", ".join(validation_errors)}')
        return Response(status_code=400, body={'message': '. '.join(messages)} if messages else {})

    @staticmethod
    def conflict(message: str = None) -> 'Response':
        return Response(status_code=409, body={'message': message} if message else {})

    @staticmethod
    def server_error(message: str = None) -> 'Response':
        return Response(status_code=500, body={'message': message} if message else {})
