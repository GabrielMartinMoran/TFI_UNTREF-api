...
class StringValidator(Validator):
    ...
    def is_valid(self, model: object) -> bool:
        ...
        return isinstance(value, str) and self.min_len <= len(value) <= self.max_len and (
                not self.fixed_len or len(value) == self.fixed_len
        ) and (not self.regex or re.compile(self.regex).match(value) is not None)
