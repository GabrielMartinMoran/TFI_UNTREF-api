...
class MockedController:
    ...
    def mocked_http_endpoint_with_user_auth_required(self):
        return Response(200, {'message': 'OK'})

    def mocked_http_endpoint_with_device_auth_required(self):
        return Response(200, {'message': 'OK'})
...
def discover_controllers_mocked(router_instance):
    Router.register_http_method({
        ...
        'user_auth_required': False, 'device_auth_required': False
    })
    Router.register_http_method({
        'type': 'GET', 'alias': None, 'class_name': 'MockedController',
        'method_name': 'mocked_http_endpoint_with_params', 'user_auth_required': False, 'device_auth_required': False
    })
    Router.register_http_method({
        'type': 'GET', 'alias': None, 'class_name': 'MockedController',
        'method_name': 'mocked_http_endpoint_with_user_auth_required', 'user_auth_required': True,
        'device_auth_required': False
    })
    Router.register_http_method({
        'type': 'GET', 'alias': None, 'class_name': 'MockedController',
        'method_name': 'mocked_http_endpoint_with_device_auth_required', 'user_auth_required': False,
        'device_auth_required': True
    })
    Router.register_http_method({
        'type': 'GET', 'alias': None, 'class_name': 'MockedController',
        'method_name': 'mocked_http_endpoint_that_raises_exception', 'user_auth_required': False,
        'device_auth_required': False
    })
...
def test_router_map_rutes_when_instantiated():
    ...
    assert not router.routes[0].methods[0].user_auth_required
...
def test_route_returns_error_response_when_user_token_is_required_and_not_provided(router):
    request = MockedRequest('GET')
    actual = router.route(request, 'mocked/mocked_http_endpoint_with_user_auth_required')
...
def test_route_returns_error_response_when_device_token_is_required_and_not_provided(router):
    request = MockedRequest('GET')
    actual = router.route(request, 'mocked/mocked_http_endpoint_with_device_auth_required')
    assert actual.body['message'] == 'Unauthorized'
    assert actual.code == 401


def test_route_returns_error_response_when_device_token_is_required_and_an_user_one_is_used_instead(router):
    request = MockedRequest('GET', {'Authorization': create_user_token('test_user@test.com')})
    actual = router.route(request, 'mocked/mocked_http_endpoint_with_device_auth_required')
    assert actual.body['message'] == 'Unauthorized'
    assert actual.code == 401
