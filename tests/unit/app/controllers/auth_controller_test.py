...
def test_auth_controllers_instantiate_user_repository_when_instantiated():
    controller = AuthController(None)
    assert controller.user_repository is not None


def test_create_returns_validation_error_when_user_in_json_body_is_not_valid():
    controller = AuthController(
        Request.from_body({'email': 'invalid_email', 'password': 'Passw0rd', 'username': 'Username'}))
    actual = controller.register()
    assert actual.status_code == 400
    assert 'email' in actual.body['message']


def test_create_returns_error_when_user_with_same_email_already_exists():
    ...
    controller.user_repository.exists = lambda x: True
    actual = controller.register()
    assert actual.status_code == 409
    assert actual.body['message'] == 'User with same email already exists'


def test_create_returns_error_when_can_not_insert_user_in_database():
    controller = AuthController(
        Request.from_body({'email': 'test@email.com', 'password': 'Passw0rd', 'username': 'Username'}))
    controller.user_repository.email_exists = lambda x: False
    controller.user_repository.create = lambda x: (
...
def test_create_returns_ok_when_user_is_created():
    controller = AuthController(
        Request.from_body({'email': 'test@email.com', 'password': 'Passw0rd', 'username': 'Username'}))
    controller.user_repository.exists = lambda x: False
    expected = 'new_user_id'
    controller.user_repository.create = lambda x: expected
    actual = controller.register()
