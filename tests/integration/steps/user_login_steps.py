...
def user_is_logged_in():
    ...
    auth_controller.get_json_body = lambda: {
        'email': user_email,
        'password': user_password
    }
    encoded_token = auth_controller.login().body['token']
    shared_variables.logged_user_token = JWTHelper.decode_token(encoded_token)
...
def user_login_fails():
    assert shared_variables.last_response.status_code == 500
